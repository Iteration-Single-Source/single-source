...
type PublicProfileDTO = { name: string; links: PublicLink[] };
...
export default function PublicProfile() {
  ...
  if (notFound) {
    return (
      <div className="max-w-xl mx-auto">
        ...
        <p className="mb-4">
          We couldn’t find <strong>@{username}</strong>. Double-check the URL or try another username.
        </p>
        <button onClick={() => navigate("/", { replace: true })} className="border rounded px-3 py-2">
          Go home
        </button>
      </div>
    );
  ...
  const { name, links = [] } = profile ?? { name: `@${username}`, links: [] };
